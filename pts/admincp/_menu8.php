...
class func_focus{
      ...
      function do_Add(){
               ...
               if ($ok){
                   ...
                   while ($data=$DB->fetch_row($query)){
                          ...
                          $queryF = "SELECT * FROM ".$conf['perfix']."menu8 WHERE cat_id='{$cat_id}' AND newsid ='{$newsid}'";
                          $resultF = $DB->query($queryF) ;
                          if ($DB->num_rows($resultF)==0){
                              $insert_sql ="INSERT INTO ".$conf['perfix']."menu8 (id,cat_id,newsid) VALUES ('','{$cat_id}','{$newsid}')";
                              $DB->query($insert_sql);
      ...
      function do_Del(){
               ...
               if ($ok){
                   $query = "DELETE FROM ".$conf['perfix']."menu8 WHERE cat_id=".$cat_id." AND newsid=-1".$qr;
                   if ($DB->query($query)){
      ...
      function do_Manage(){
               ...
               // List All Focus
               $query = $DB->query("SELECT * FROM ".$conf['perfix']."menu8 $where_focus order by newsid,id DESC");
               ...
               for ($i1=1; $i1<$num_pages1+1; $i1++ ) {
                    ...
                    else $nav.="[<a href='?act=menu8&sub=manage{$ext}&p1={$i1}'>$i1</a>] ";
               }
               $nav .= "</div><center>";
               $list = "";
               $stt=0;
               $sql= "SELECT * FROM ".$conf['perfix']."menu8 $where_focus order by newsid DESC LIMIT $start1,$n";
               $query = $DB->query($sql);
               while ($data=$DB->fetch_row($query)) {
                      $data['stt'] = $stt;
               ...
               for ($i=1; $i<$num_pages+1; $i++ ) {
                    ...
                    else $nav.="[<a href='?act=menu8&sub=manage{$ext}&p={$i}'>$i</a>] ";
               }
               $nav .= "</div></center>";
               $list = "";
               $stt=0;
               ...
               return<<<EOF
                       ...
                       <br><table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
                           ...
                           <tr>
                              ...
                              <form action="?act=menu8&cat_id={$data['cat_id']}&sub=add" method="post" name="manage" id="manage">
...
EOF;
        ...
        function html_nav1($data){
                 return<<<EOF
                          ...
                          <br><table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
                              <form action="?act=menu8&sub=manage" method="post" name="myform">
                                  <tr>
                                  ...
                                  <tr><td>&nbsp;</td></tr>
                                      ...
                                      <tr>
                                          <td bgcolor="#FFFFFF" class="main_table" align=center>
                                          <form action="?act=menu8&sub=del&cat_id={$data['cat_id']}" method="post" name="manage1" id="manage1">
                                              <table width="100%" border="0" cellspacing="1" cellpadding="1">
                                                 <tr>
                                                     <td width="10%" class="row_tittle">Select</td>
                                                     <td width="55%" class="row_tittle">News Title</td>
